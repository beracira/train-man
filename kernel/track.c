...
void init_data(int t) {
  volatile track_node * track = (track_node *) TRACK_ADDR;
  if (t == TRACK_A) {
    init_tracka((track_node *) track); // cast to make warnings go away
  } else {
...
// init all switches to curved
void track_init_switch(track_node *track) {
  int i = 0;
  for (i = 80; i <= 122; i += 2) {
    track[i].dir = DIR_CURVED;
  }
}
